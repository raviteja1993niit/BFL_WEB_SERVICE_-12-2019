...
public class OracleAQConfig {
	private static Logger logger = Logger.getLogger(BajajRestController.class.getName());
	@Autowired
	...
	public JmsListenerContainerFactory<?> jmsListenerContainerFactory(@Autowired
			@Qualifier("aqjmsConnectionFactory")
			QueueConnectionFactory queueConnectionFactory,@Autowired
			@Qualifier("pooledConnectionFactory")PooledConnectionFactory pooledConnectionFactory, DefaultJmsListenerContainerFactoryConfigurer configurer) {
		DefaultJmsListenerContainerFactory factory = new DefaultJmsListenerContainerFactory();
		factory.setMessageConverter(jacksonJmsMessageConverter());
		if(Boolean.parseBoolean(env.getProperty("isActiveMQ")) == true)
		{
